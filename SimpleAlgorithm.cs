...
namespace EDT_EAH_Algorithm
{
    public class SimpleAlgorithm
    {
        public List<string> Compute(Database pSource)
        {
            ...
            foreach (var lStudent in lDayStudents)
            {
                foreach (var lFieldArea in lStudent.FieldAreas)
                {
                    // Look for all teachers which can do this field.
                    List<Teacher> lCorrectTeachers = lDayTeachers.Where(pTeacher => pTeacher.FieldAreas.Contains(lFieldArea)).ToList();
                    List<IntVar> lAllHourDecisions = new List<IntVar>();
                    foreach (var lTeacher in lCorrectTeachers)
                    {
                        ...
                        lAllHourDecisions.AddRange(lHourDecisions);
                    }

                    if (lAllHourDecisions.Count != 0)
                    {
                        lModel.Add(LinearExpr.Sum(lAllHourDecisions.ToArray()) <= 1);
                    }
                }
            }
